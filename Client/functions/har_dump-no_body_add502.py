...
def error_occurred(flow: mitmproxy.http.HTTPFlow):
    """
    Called when a flow encounters an error.
    """
    try:
        if flow.error:
            failed_flow_entry(flow, flow.error)
    except Exception as e:
        ctx.log.error(f"Error processing error event: {e}")


def failed_flow_entry(flow: mitmproxy.http.HTTPFlow, exception: Exception) -> dict:
...
def response(flow: mitmproxy.http.HTTPFlow):
    """
    Called when a server response has been received or failed.
    """
    try:
        if flow.exception:
            failed_flow_entry(flow, flow.exception)
        else:
            flow_entry(flow)
    except Exception as e:
