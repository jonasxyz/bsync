...
const statFunctions = require('./functions/statFunctions.js');
const logFunctions = require('./functions/logging.js');
const helperFunctions = require('./functions/helper.js');
...
const urlList = fs.readFileSync("./" + config.url_list).toString().split("\r\n"); // create array with one element out of each line
...
io.on("connection", (socket) => {
    if (activeClients >= config.num_clients) {
        ...
        logFunctions.consoleError("Too many clients connected. Check config.num_clients argument.");
        io.to(socket.id).emit("close", "toomanyclients");
    }

    activeClients += 1; // counting connected clients
    ...
    socket.on("initialization", (clientname) => {
        ...
        // creating two arrays for managing timings in the calibration and crawl process
        ...
        if (activeClients == config.num_clients && ongoingCrawl == false) { // start over with pingtest when all clients connected
        ...
        if (ongoingCrawl == true && activeClients == config.num_clients) { // watch for reconnected clients while crawling
    ...
    socket.on("disconnect", async () => {
        ...
        let tempId = helperFunctions.searchArray(arrayClients, socket.id.toString(), 2);
        ...
        if (ongoingCrawl == true && activeClients < config.num_clients) {
            ...
            logFunctions.consoleError("Client " + socket.data.clientname + " disconnected while crawling...\nClient will automatically  try to reconnect.");
    ...
    socket.on("pingresults", (data) => { // collecting answers from ping signal for calulating latency
        ...
        if (latency < config.allowed_ping) {
            ...
            logFunctions.consoleError("Delay between Client and Master is too high. Edit var allowedDelay if necessary.");
    ...
    socket.on("scripterror", (data) => {
        logFunctions.consoleError("Error at " + socket.data.clientname + " while executing crawl script");
        // console.log("\x1b[31mERROR: " + "Error at "+ socket.data.clientname+ " while executing crawl script","\x1b[0m");
        // console.log(data); // debug
    });

    socket.on("urlcrawled", (data) => { // listens for succesfull visit of current URL
        ...
        awaiting = "browserFinished";

    });

    socket.on("browserfinished", async () => { // signal triggered when workers browser closed

        if (activeClients != config.num_clients || ongoingCrawl == false || browsersReady != config.num_clients || awaiting != "browserFinished") return;
        ...
        // check if browser closed but didnt send urlcrawled signal
        if (tempDateUrlDone == undefined) {
            tempDateUrlDone = -1;
            // console.log("\x1b[31mERROR: " + socket.data.clientname.toString() + " timed out visiting URL....", "\x1b[0m");
            logFunctions.consoleError(socket.data.clientname.toString() + " timed out visiting URL....");
    ...
    socket.on("browserready", (data) => { // listening for successful launch of workers browser
        ...
        if (browsersReady == config.num_clients) { // if all workers signalized that their browser is ready
            ...
            io.emit("browsergo"); // broadcast signal to access the url after all browser are ready
    ...
    function sendUrl(calibration) { // distributing URL to all workers

        awaiting = "browserReady";
        ongoingCrawl = true;
        pendingJobs = 0;
        pendingRequests = 0;
        browsersReady = 0;
    ...
    function browserDoneTimeout() {
        ...
        logFunctions.consoleError("Client " + tempName + " browser timed out while visiting URL...\nKilling all browsers.");
...
function browserReadyTimeout() { // timeout if the browser ready signal ist not received within the timelimit
    ...
    if (tempId != undefined) { // kill and restart timed out browser
        ...
        logFunctions.consoleError("Client " + tempName + " browser timed out while starting...\nTrying to kill childprocess.");
