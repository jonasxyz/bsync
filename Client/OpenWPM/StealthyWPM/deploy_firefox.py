...
def deploy_firefox(
    ...
    # Set proxy and useragent for bsync
    configure_firefox.bsyncoptions(browser_params, prefs)
    ...
    # set window size
    if browser_params.screen_properties:
        with open(os.path.join(root_dir, 'screen.txt'), 'r') as f:
            # Set resolution
            line = f.readline()
            resolution = tuple(line.strip().split(','))
            print(resolution)
            driver.set_window_size(*resolution, windowHandle="current")
            # Set position
            line = f.readline()
            position = tuple(line.strip().split(','))
            driver.set_window_position(*position, windowHandle="current")
    else:
        driver.set_window_size(*DEFAULT_SCREEN_RES, windowHandle="current")
