...
function createClientSubpages(clientId, getCalibrationDone, arrayClients, arrayStatistics, getTimeUrlSent, getTimeAllBrowsersReady) { 
    ...
    app.get(`/client/${clientId}`, (req, res) => {
        ...
        let whichArray;
        const isCalibration = typeof getCalibrationDone === 'function' ? getCalibrationDone() : false;
        isCalibration ? (whichArray = arrayStatistics) : (whichArray = arrayClients);

        // Find the client's position in the array
        let arrayPosition = helperFunctions.searchArray(whichArray, clientId, 2);

        // Store timing information
        const tUrlSent = typeof getTimeUrlSent === 'function' ? getTimeUrlSent() : undefined;
        const tAllReady = typeof getTimeAllBrowsersReady === 'function' ? getTimeAllBrowsersReady() : undefined;
        if (typeof tUrlSent === 'number') {
            whichArray[arrayPosition].requestArray.push((accessTime - tUrlSent));
        } else {
            whichArray[arrayPosition].requestArray.push(undefined);
        }
        ...
        let sinceStart = typeof tUrlSent === 'number' ? (accessTime - tUrlSent) : 'n/a';
        let sinceAllReady = typeof tAllReady === 'number' ? (accessTime - tAllReady) : 'n/a';
        console.log("\x1b[36mREQUEST:\x1b[0m HTTP Request from " + tempName + " \x1b[36m" + sinceStart + "\x1b[0m ms after starting iteration. ", sinceAllReady
